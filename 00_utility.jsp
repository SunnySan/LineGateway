...
//取得某個Channel的設定資料
public Hashtable getChannelProfile(String sChannelName){
	//sSQL是SQL statement
	//iColCount是ResultSet中每個row的column數
	...
	sSQL = "SELECT Channel_Secret, Channel_Access_Token, Service_Url, Line_ID, QRCode_File";
	sSQL += " FROM line_gateway_channels";
	sSQL += " WHERE Line_Channel_Name='" + sChannelName + "'";
	...
	if (sResultCode.equals(gcResultCodeSuccess)){	//有資料
		s = (String[][])ht.get("Data");
		htResponse.put("Channel_Secret", nullToString(s[0][0], ""));
		htResponse.put("Channel_Access_Token", nullToString(s[0][1], ""));
		htResponse.put("Service_Url", nullToString(s[0][2], ""));
		htResponse.put("Line_ID", nullToString(s[0][3], ""));
		htResponse.put("QRCode_File", nullToString(s[0][4], ""));
	}
...
/*********************************************************************************************************************/
//比對Line 送過來的 HTTP header 中 X-Line-Signature 與我方計算出的是否相同
public java.lang.Boolean compareLineSignature(String sLineSignature, String sMySecret, String sBody){
	java.lang.Boolean bOK = false;

	try {
		Mac mac = Mac.getInstance("HmacSHA256");
		SecretKeySpec secret = new SecretKeySpec(sMySecret.getBytes(),"HmacSHA256");	//Line發給此服務的 channel secret
		mac.init(secret);
		byte[] digest = mac.doFinal(sBody.getBytes());	//用 HmacSHA256 將 HTTP Post 的 Body 內容產出 digest
		String enc = new String(Base64.encodeBase64(digest));	//將 digest 做 Base64 編碼
		if (enc.equals(sLineSignature)){	//與 HTTP header 中的 Signature 比對
			bOK = true;
			writeLog("debug", "Signature match!");
		}else{
			writeLog("error", "Signature mismatch, my signature=" + enc);
		}	//if (!enc.equals(sSignature)){	//與 HTTP header 中的 Signature 比對
	} catch (Exception e) {
		writeLog("error", "Exception when compare Line signature= " + e.getMessage());
	}	//try {
